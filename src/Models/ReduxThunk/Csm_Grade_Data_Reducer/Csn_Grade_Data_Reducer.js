...
const Csm_Grade_Data_Async = createAsyncAction(CSM_GRADE_DATA_GET_START, CSM_GRADE_DATA_GET_SUCCESS, CSM_GRADE_DATA_GET_ERROR)();

const Csm_Grade_Count_Data_Getting_Axios = async Filter_Data => {
    try {
        const Csm_Regi_Data = await request.get(`/CE_Calendar_app_server/Csm_Grade_Count_Data_Getting`, {
            params: {
                State: Filter_Data.csm_basic_data_state,
            },
        });
...
export function Csm_Grade_Data_Reduce_Thunk(Filter_Data) {
    return async dispatch => {
        const { request, success, failure } = Csm_Grade_Data_Async;
        dispatch(request());
        try {
            const userProfile = await Csm_Grade_Count_Data_Getting_Axios(Filter_Data);
            if (userProfile) {
