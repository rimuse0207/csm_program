...
//                 const Update_Data_Key = Csm_Data.map((list) => list.csm_basic_data_csm_key === data.csm_basic_data_csm_key ? { ...list,csm_custom_id:Login_Info.Login_id,csm_custom_name: Login_Info.Login_name,csm_custom_csm_key:data.csm_basic_data_csm_key } : list );
...
//     },[data])
    ...
    return (
        <PublishMainDivBox>
            { data.csm_publish_id && data.csm_apply_id && data.csm_entering_id && data.csm_ce_id ?<div>{data.csm_custom_id ? <div> <div>{data.csm_custom_name}</div>
                <div>{ moment(data.csm_custom_write_date).format("YY-MM-DD")}</div></div>:<div></div>}</div>:<div></div>}
