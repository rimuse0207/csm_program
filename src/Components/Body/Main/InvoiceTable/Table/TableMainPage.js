...
const TableMainPage = () => {
    ...
    const Handle_Invoice_Print = async () => {
        try {
            ...
            if (Handle_Invoice_Print_Axios.data.dataSuccess) {
                ...
                const {
                    csm_invoice_print_select_date,
                    csm_invoice_print_key,
                    csm_invoice_print_travel_fee_unit,
                    csm_invoice_print_travel_fee_price,
                    csm_invoice_print_hotel_fee_unit,
                    csm_invoice_print_hotel_fee_price,
                    csm_invoice_print_service_fee_unit,
                    csm_invoice_print_service_fee_sum_price,
                    csm_invoice_print_total_fee,
                } = Handle_Invoice_Print_Axios.data.Handle_Invoice_Print_Getting_Rows[0];

                window.open(
                    `/Invoice_Printer/${moment(csm_invoice_print_select_date).format(
                        'YYYY-MM-DD'
                    )}/${csm_invoice_print_key}/${csm_invoice_print_travel_fee_price}/${csm_invoice_print_hotel_fee_unit}/${csm_invoice_print_hotel_fee_price}/${csm_invoice_print_service_fee_unit}/${csm_invoice_print_total_fee}`,
                    '_blank',
                    options
                );
            } else {
    ...
    const Handle_Delete_Invoice_Number = async () => {
        ...
        try {
            ...
            try {
                const Delete_Invoice_Number_Handle_Clicks_Axios = await request.post(
                    `/CE_Calendar_app_server/Delete_Invoice_Number_Handle_Clicks`,
                    {
                        RightMenuClickKeys,
                ...
                <tbody>
                        ...
                        return (
                            <>
                                    ...
                                    style={
                                        RightMenuIsOpen
                                            ? RightMenuClickKeys?.Invoice_Sum_List_Rows.csm_invoice_user_list_registration_key ===
                                              list.Invoice_Sum_List_Rows.csm_invoice_user_list_registration_key
                                                ? {}
                                                : { opacity: '0.5' }
                                            : {}
                                    }
                                >
                                    ...
                                    <TableData rowSpan={Sum_Lenght}>
                                        {list?.Invoice_Sum_List_Rows?.csm_invoice_list_erp_document_number}
                                    </TableData>
                                    <TableData rowSpan={Sum_Lenght}>
                                        ￥
                                        {list.Inovoice_basic_csm_Data.reduce((acc, innerArray) => {
                                            return (
                                                acc + innerArray.reduce((innerAcc, obj) => innerAcc + obj.csm_user_input_data_total_cost, 0)
                                            );
                                        }, 0).toLocaleString()}
                                    </TableData>
                                </TableRow>
                                    ...
                                    return item.map((data, k) => {
                                        return (
                                            <TableRow
                                                ...
                                                style={
                                                    RightMenuIsOpen
                                                        ? RightMenuClickKeys?.Invoice_Sum_List_Rows
                                                              .csm_invoice_user_list_registration_key ===
                                                          list.Invoice_Sum_List_Rows.csm_invoice_user_list_registration_key
                                                            ? {}
                                                            : { opacity: '0.5' }
                                                        : {}
                    ...
                    {RightMenuIsOpen ? (
                        ...
                        >
                            <ul>
                                {RightMenuClickKeys?.Invoice_Sum_List_Rows.csm_invoice_user_list_registration_key ? (
                                    RightMenuClickKeys?.Invoice_Sum_List_Rows.csm_invoice_list_erp_document_number ? (
                                        <li onClick={() => Handle_Delete_Document_Number()}>전표번호 삭제</li>
                                    ) : (
                                        <>
