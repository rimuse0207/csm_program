...
const FilterBinds = ({Close_Filter_Modal,SelectMenuTitle}) => {
    ...
    const handleClick_Filter_State = (Select) => {
        dispatch(Csm_Filtering_Change_Data({ ...Csm_Filter_States, csm_basic_data_binds: Select }))
        Close_Filter_Modal();
    }
                ...
                <div className="List_Data_Container">
                    ...
                    <ul>
                        {List_Csm_State_Data.map((list) => {
                            return <li>
                                <div onClick={()=>handleClick_Filter_State(list.csm_basic_data_binds)}>
