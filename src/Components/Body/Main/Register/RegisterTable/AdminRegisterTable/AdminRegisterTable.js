...
import {
    Csm_Register_Data_Change_Checked,
    Csm_Register_Data_Reduce_Thunk,
} from '../../../../../../Models/ReduxThunk/Csm_Regi_Data_Reducer/CsmRegiDataReducer';
import {
    Csm_Invoice_Select_Add_Data,
    Csm_Invoice_Select_Delete_Data,
} from '../../../../../../Models/Csm_Select_Reducer/CsmInvoiceSelectReducer';
...
const AdminRegisterTable = () => {
    ...
    const handle_Add_Invoice_Redux = data => {
        if (data.Main_Data.checked === 'false' || !data.Main_Data.checked) {
            ...
            const Change_Checked_True = Csm_Register_State?.Regi_Csm_Data.map(list =>
                list.Main_Data.csm_basic_data_csm_key === data.Main_Data.csm_basic_data_csm_key
                    ? {
                          ...list,
                          Main_Data: {
                              ...list.Main_Data,
            ...
            const Delete_Csm_Invoice_Select_State = Csm_Invoice_Select_State.filter(list =>
                list.Main_Data.csm_basic_data_csm_key === data.Main_Data.csm_basic_data_csm_key ? '' : list
            );
            const Change_Checked_False = Csm_Register_State?.Regi_Csm_Data.map(list =>
                list.Main_Data.csm_basic_data_csm_key === data.Main_Data.csm_basic_data_csm_key
                    ? {
                          ...list,
                          Main_Data: {
    ...
    return (
        <NewRegisterTableMainDivBox>
                ...
                <ul className="Table_Header">
                    <div className="Main_Header_Table_Container">
                        ...
                        <li style={{ width: '100px' }}>
                            선택
                            <br />({Csm_Invoice_Select_State.length})
                        </li>
            ...
            <div>
                {Csm_Register_State?.Regi_Csm_Data?.map((list, j) => {
                    return (
                        <ul
                            ...
                            style={
                                RightMenuIsOpen
                                    ? RightMenuClickKeys?.Main_Data.csm_user_input_list_registration_key ===
                                      list.Main_Data.csm_user_input_list_registration_key
                                        ? {}
                                        : { opacity: '0.5' }
                                    : {}
                            }
                        >
                            <div className="Main_Body_Table_Container">
                                <li style={{ textAlign: 'center', width: '50px' }}>{j + 1}.</li>
                                <li style={{ width: '100px' }}>
                                    <input
                                        type="checkbox"
                                        disabled={list.Main_Data.csm_invoice_list_registration_key ? true : false}
                                        checked={list.Main_Data.checked === 'false' || !list.Main_Data.checked ? false : true}
                                        onChange={() => handle_Add_Invoice_Redux(list)}
                                        style={list.Main_Data.csm_pay_id ? { opacity: 0.2 } : {}}
                                    ></input>
                                </li>
                                <li style={{ width: '100px' }}>
                                ...
                                <li style={{ width: '100px' }}>
                                    <div>
                                        ￥{list.Main_Data.csm_user_input_data_total_cost.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                                    </div>
                                </li>
                            </div>
                                    ...
                                    {list.Sub_Data.map(item => {
                                        return (
                                            <div className="Sub_Data_List">
                                                ...
                                                <li>
                                                    {item.csm_user_input_data_stay_days === 0
                                                        ? ''
                                                        : item.csm_user_input_data_stay_days + '일'}{' '}
                                                </li>
                                                <li>
                                                    {item.csm_user_input_data_travel_range_cost !== 0
                                                        ? '￥' +
                                                          item.csm_user_input_data_travel_range_cost
                                                              .toString()
                                                              .replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                                                        : ''}
                                                </li>
                                                <li>
                                                    {item.csm_user_input_data_travel_time_cost !== 0
                                                        ? '￥' +
                                                          item.csm_user_input_data_travel_time_cost
                                                              .toString()
                                                              .replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                                                        : ''}
                                                </li>
                                                <li>
                                                    {item.csm_user_input_data_stay_days_cost !== 0
                                                        ? '￥' +
                                                          item.csm_user_input_data_stay_days_cost
                                                              .toString()
                                                              .replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                                                        : ''}
                                                </li>
                                                <li>
                                                    {item.csm_user_input_data_operation_cost !== 0
                                                        ? '￥' +
                                                          item.csm_user_input_data_operation_cost
                                                              .toString()
                                                              .replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                                                        : ''}
                                                </li>
                ...
                {RightMenuIsOpen ? (
                    <div
                        className="Right_Menu_Container"
                        style={{ position: 'fixed', top: `${RightMenuPosition.y}px`, left: `${RightMenuPosition.x}px` }}
                    >
                        <ul>
                            {RightMenuClickKeys.Main_Data.csm_pay_id ? (
                                <li style={{ opacity: '0.5' }}>사용자 등록 취소</li>
                            ) : (
                                <li onClick={() => handle_Delete_Complete_Data_Confirm_Checking()}>사용자 등록 취소</li>
                            )}
                        </ul>
                    </div>
                ) : (
